...
function root() { const dir = mkdtempSync(join(tmpdir(), "pi-stress-1000-")); roots.push(dir); ensureMemoryDirs(dir); return dir; }
...
describe("retrieval stress 1000 records", () => {
  test("builds context under local target with qmd disabled", async () => {
    const r = root();
    for (let i = 0; i < 10; i++) addMemoryRecord(r, rec(`l1_${i}`, "L1", `identity ${i}`));
    for (let i = 0; i < 990; i++) addMemoryRecord(r, rec(`l2_${i}`, "L2", i < 20 ? `relevant qmd retrieval ${i}` : `unrelated graphics ${i}`));
