...
describe("PatchReviewPanel editing", () => {
  test("updates highlighted operation statement", () => {
    const p = patch();
    const panel = new PatchReviewPanel(p, () => {});
    expect(panel.editHighlightedStatement("Edited statement")).toBe(true);
    expect(p.ops[0].record?.statement).toBe("Edited statement");
    expect(panel.render(80).join("\n")).toContain("Edited statement");
  });
});
