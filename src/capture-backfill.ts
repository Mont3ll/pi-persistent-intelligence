...
function fingerprintPayload(since: string | undefined, candidates: CaptureCandidate[], contaminated: string[], contaminatedCandidates: string[], rescope: string[], skippedAmbiguousScope: string[]): string {
  return JSON.stringify({
    ...
    candidates: candidates.map((candidate) => ({
      ...
      recurrence_count: candidate.recurrence_count ?? 1,
      evidence_refs: [...candidate.evidence_refs].sort(),
      source_session_ids: [...(candidate.source_session_ids ?? [])].sort(),
      primary_trust_class: candidate.primary_trust_class,
      source_trust_weight: candidate.source_trust_weight,
